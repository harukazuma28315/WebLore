...
document.addEventListener('click', function(e) {
  document.querySelectorAll('.dropdown-tag').forEach(function(dropdown) {
    if (!dropdown.contains(e.target)) {
      ...
      dropdown.querySelector('.dropdown-tag-label').classList.remove('active');
    }
  });
});
...
document.querySelectorAll('.dropdown-tag').forEach(function(dropdown) {
  ...
  const checkboxes = dropdown.querySelectorAll('.dropdown-tag-list input[type="checkbox"]');

  checkboxes.forEach(function(cb) {
    cb.addEventListener('change', function() {
      const checked = dropdown.querySelectorAll('.dropdown-tag-list input[type="checkbox"]:checked');
      if (checked.length > 0) {
        let selected = [];
        ...
        label.innerHTML = selected.join(', ') + ' <i class="fa fa-caret-down" style="float:right;"></i>';
      } else {
        label.innerHTML = 'Tag <i class="fa fa-caret-down" style="float:right;"></i>';
      }
    });
  });
});
